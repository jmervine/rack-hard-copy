...
class TestStaticSave < MiniTest::Unit::TestCase
  class MockApp
    def call(env = nil)
      [200, {}, ["hello"]]
    end
  ...
  def setup
    @middleware = Rack::Static::Save.new(MockApp.new, :store => "/tmp/static_copy_minitest", :headers => true)
  end
  ...
  def test_init_creates_static_directory
    Rack::Static::Save.new(MockApp.new, :store => "/tmp/static_copy_minitest", :headers => true)
    assert File.directory?("/tmp/static_copy_minitest")
  end

  def test_appends_index_html
    env = { 'PATH_INFO' => '/foo/bar', 'REQUEST_METHOD' => 'GET' }
    assert_expected_response @middleware.call(env), { 'X-Rack-Static-Save' => 'true' }
  ...
  def test_with_ignores
    @middleware = Rack::Static::Save.new(MockApp.new, :store => "/tmp/static_copy_minitest", :ignores => [ "txt" ], :headers => true)
    env = { 'PATH_INFO' => '/foo/bar.txt', 'REQUEST_METHOD' => 'GET' }
    assert_expected_response @middleware.call(env), { 'X-Rack-Static-Save' => 'false' }
    refute File.exists?("/tmp/static_copy_minitest/foo/bar.txt")
  end

  def test_not_expired
    @middleware = Rack::Static::Save.new(MockApp.new, :store => "/tmp/static_copy_minitest", :headers => true, :timeout => 600)
    env = { 'PATH_INFO' => '/foo/bar.txt', 'REQUEST_METHOD' => 'GET' }
  ...
  def test_expired
    @middleware = Rack::Static::Save.new(MockApp.new, :store => "/tmp/static_copy_minitest", :headers => true, :timeout => 0)
    env = { 'PATH_INFO' => '/foo/bar.txt', 'REQUEST_METHOD' => 'GET' }
    `mkdir -p /tmp/static_copy_minitest/foo/ && touch /tmp/static_copy_minitest/foo/bar.txt`
    assert_expected_response @middleware.call(env), { 'X-Rack-Static-Save' => 'true' }
  end
